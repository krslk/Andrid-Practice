...
public interface ScoreMapper {
    ...
    // 分页
    @Select("select scoreId,examCode,studentId,subject,ptScore,etScore,score,answerDate from score where studentId = #{studentId} order by scoreId desc limit #{offset},#{size}")
    List<Score> findById(Long offset, Long size, Integer studentId);

    // 不分页
    @Select("select scoreId,examCode,studentId,subject,ptScore,etScore,score,answerDate from score where studentId = #{studentId}")
    List<Score> noPageFindById(Integer studentId);
